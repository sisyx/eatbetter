...
const Users = () => {
    ...
    const {data: users, isError: error, isLoading: loading, } = useGetData(["users", reload], getUsers);
            ...
            <div className="flex flex-col gap-4 w-full h-full">
                ...
                <hr />
                {
                    ...
                    <div className='flex flex-col gap-2 pb-12'>
                        {users?.map((user: UserType) => <User {...user} username={user.username} reloadFn={reloadFn} />)}
                    </div>
                }
            </div>
        </Layout>
